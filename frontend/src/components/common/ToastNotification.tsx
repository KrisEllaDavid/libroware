...
const ToastNotification: React.FC = () => {
  ...
  const getToastClasses = (type: ToastType): string => {
    const baseClasses =
      "p-4 rounded-lg shadow-xl flex items-center justify-between text-white max-w-md w-full mx-auto transform";
  ...
  return (
    <>
      ...
      <div
        className="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm z-[90] pointer-events-none transition-opacity duration-300 animate-backdrop-appear"
        aria-hidden="true"
      ></div>

      {/* Toasts container - positioned just below the navigation bar */}
      <div className="fixed top-20 inset-x-0 z-[95] flex flex-col items-center gap-3 px-4">
        {toasts.map((toast) => (
          <div
            key={toast.id}
            className={`${getToastClasses(
              toast.type
            )} animate-toast-drop ring-opacity-30 shadow-2xl overflow-hidden relative`}
            role="alert"
          >
