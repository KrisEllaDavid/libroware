...
const UserActivity: React.FC = () => {
  ...
  if (loading) return <div className="flex justify-center py-8">Loading your activity...</div>;
  if (error) return <div className="text-red-500">Error loading your activity: {error.message}</div>;
  ...
  return (
    <div className="p-4 sm:p-6 bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-100 dark:border-gray-700">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">My Book Requests</h1>
        <p className="text-sm text-gray-500 dark:text-gray-400">View your pending and completed requests</p>
      </div>
        ...
        <nav className="-mb-px flex space-x-8" aria-label="Tabs">
          ...
          >
            ...
            {pendingBorrows.length > 0 && (
              <span className={`ml-2 px-2 py-0.5 rounded-full text-xs font-medium 
                ${activeTab === 'pending' ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900 dark:text-emerald-200' : 
          ...
          >
            ...
            {historyBorrows.length > 0 && (
              <span className={`ml-2 px-2 py-0.5 rounded-full text-xs font-medium 
                ${activeTab === 'history' ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900 dark:text-emerald-200' : 
                'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-200'}`}>
          ...
          <>
            {pendingBorrows.length === 0 ? (
              <div className="text-center py-10">
                ...
                <div className="mt-6">
                  <a href="/books" className="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-emerald-600 hover:bg-emerald-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-emerald-500">
                    Browse Library
                  </a>
                </div>
              </div>
                        ...
                        <td className="px-4 py-4 whitespace-nowrap">
                          <div className="flex items-center">
                            {borrow.book.coverImage ? (
                              ...
                              <div className="w-10 h-14 bg-gray-200 dark:bg-gray-700 rounded-sm mr-3 flex items-center justify-center text-gray-500 dark:text-gray-400 text-xs">
                                No image
                              </div>
                            )}
                            <div>
          ...
          <>
            {historyBorrows.length === 0 ? (
              <div className="text-center py-10">
                ...
                <h3 className="mt-2 text-sm font-medium text-gray-900 dark:text-white">No request history</h3>
                <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
