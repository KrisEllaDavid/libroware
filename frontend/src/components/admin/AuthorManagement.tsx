...
import { CREATE_AUTHOR, UPDATE_AUTHOR, DELETE_AUTHOR } from '../../graphql/mutations';
...
const AuthorManagement: React.FC = () => {
  ...
  const { loading, error: queryError, data, refetch } = useQuery(GET_AUTHORS, {
  ...
  const [deleteAuthor, { loading: deleteLoading }] = useMutation(DELETE_AUTHOR, {
    onCompleted: () => {
      refetch();
    },
    onError: (error: any) => {
  ...
  const renderContent = () => {
    ...
    if (loading) {
      return (
        <div className="p-6 text-center">
          ...
          <p className="mt-2 text-sm text-gray-500 dark:text-gray-400 transition-colors">Loading authors...</p>
        </div>
      );
    }
    ...
    if (authors.length === 0) {
      return (
        <div className="p-6 text-center">
          <p className="text-sm text-gray-500 dark:text-gray-400 transition-colors">No authors found. Click "Add Author" to create one.</p>
        </div>
      );
    }
    ...
    return (
      <ul className="divide-y divide-gray-200 dark:divide-gray-700">
        {authors.map((author) => (
          <li key={author.id} className="px-4 py-4 hover:bg-gray-50 dark:hover:bg-gray-700 sm:px-6 transition-colors">
            <div className="flex items-center justify-between">
              <div>
                <h4 className="text-sm font-medium text-gray-900 dark:text-white transition-colors">{author.name}</h4>
                {author.createdAt && (
                  <p className="text-xs text-gray-500 dark:text-gray-400 transition-colors">Added: {formatDate(author.createdAt)}</p>
                )}
              </div>
